...
export default function DecisionDetailsPage() {
  ...
  async function handleSubmitForReview() {
    ...
    const confirmed = window.confirm(
      "Submit this decision for review?\n\nThe decision status will change from Draft to Under Review.",
    );
    ...
    try {
      const updatedDecision = await updateDecisionStatus(
        decision.id,
        {
          status: "Under Review",
        },
      ...
      if (status === 400) {
        setErrorMessage(
          backendMessage ||
            "This decision cannot be submitted for review in its current state.",
        );
      } else if (status === 401) {
        setErrorMessage(
          "Your session has expired. Please sign in again.",
        ...
        setErrorMessage(
          "You do not have permission to submit this decision for review.",
        );
      } else if (status === 404) {
        setErrorMessage(
        ...
        setErrorMessage(
          backendMessage ||
            "The submitted decision status is not valid.",
        );
      } else if (status && status >= 500) {
        setErrorMessage(
          "The server is temporarily unavailable. Please try again.",
        ...
        setErrorMessage(
          "Unable to submit the decision for review. Please try again.",
        );
      }
    } finally {
  ...
  const normalizedStatus = decision.status
    .trim()
    .toLowerCase();
  ...
  const canEdit =
    isAdmin || (isOwner && normalizedStatus === "draft");
  const canDelete = canEdit && normalizedStatus === "draft";
  ...
  async function handleDeleteDecision() {
    ...
    const decisionToDelete = decision;

    if (
      !window.confirm(
        "Delete this draft decision? This cannot be undone.",
      )
    ) {
      return;
    }
  ...
  return (
    <div className="decision-details-page">
      ...
      {errorMessage && (
        <div
          className="decision-details-action-alert"
          role="alert"
        >
          <Alert variant="error">{errorMessage}</Alert>
        </div>
          ...
          <section className="decision-details-side-card">
            ...
            <div className="decision-details-actions">
                ...
                >
                  {isSubmitting ? (
                    <Loader2
                      size={16}
                      aria-hidden="true"
                      className="decision-details-spinner"
                    />
                  ) : (
                    <Send
                      size={16}
                  ...
                  <span>
                    ...
                    <small>
                      Send this draft to the review workflow.
                    </small>
                  </span>
                </button>
              )}
              ...
              {canDelete && (
                <button
                  type="button"
                  className="decision-details-action-button decision-details-action-danger"
                  onClick={() => void handleDeleteDecision()}
                >
                  <Trash2 size={16} aria-hidden="true" />
                  <span>
                    <strong>Delete Draft</strong>
                    <small>
                      Remove this draft permanently.
                    </small>
                  </span>
                </button>
              )}
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    Review and compare available decision
                    alternatives.
                  </small>
                </span>
              </button>
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    View and participate in the decision
                    discussion.
                  </small>
                </span>
              </button>
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    Track reviewer assignments and approval
                    progress.
                  </small>
                </span>
              </button>
