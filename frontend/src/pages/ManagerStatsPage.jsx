...
export const ManagerStatsPage = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (isAdmin) {
        setActivity(responses[1].data?.user_stats?.by_role || []);
        setUsers(responses[2].data?.users || []);
      }
    } catch (error) {
      toast.error(error.formattedMessage || 'Unable to load statistics');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ color: 'var(--color-ink-muted-48)' }}>
            Decision volume and status health for {stats?.scope || 'your scope'}.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
        {cards.map(([label, value]) => (
          <div className="apple-card" key={label}>
            <BarChart3 size={18} color="var(--color-primary)" />
            <div style={{ fontSize: '28px', fontWeight: 600, marginTop: '12px' }}>
              {value || 0}
            </div>
            <div style={{ color: 'var(--color-ink-muted-48)', fontSize: '13px' }}>
              {label}
            </div>
          </div>
        ))}
      ...
      {isAdmin && (
        <>
          <div className="apple-card">
            ...
            <Table
              columns={[
                { key: 'role', header: 'Role', render: (value) => <Badge variant="role">{value}</Badge> },
